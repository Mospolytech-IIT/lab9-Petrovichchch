...
@app.get("/user/delete/{username}", tags=["Удаление"])
def delete_user(username: str):
    ...
    try:
        ...
        session.commit()
        session.close()
        return HTMLResponse(f"<h1>Пользователь {username} и его посты удалены.</h1>")
    except Exception as e:
        session.rollback()
        session.close()
        return HTMLResponse(f"<h1>Ошибка при удалении пользователя: {e}</h1>")


@app.get("/post/delete/{title}", tags=["Удаление"])
def delete_post(title: str):
    'удаляет все посты с указанным названием'
    session = Session()
    try:
        posts_to_delete = session.query(Posts).filter(Posts.title == title).all()
        for post in posts_to_delete:
            session.delete(post)
        session.commit()
        session.close()
        return HTMLResponse(f"<h1>Посты с названием '{title}' удалены.</h1>")
    except Exception as e:
        session.rollback()
        session.close()
        return HTMLResponse(f"<h1>Ошибка при удалении постов: {e}</h1>")
